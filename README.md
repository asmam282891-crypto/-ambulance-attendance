...
واجهة التطبيق عربية بالكامل وباتجاه من اليمين إلى اليسار، بما في ذلك لوحة
المدير. أزيل خيار تبديل اللغة حتى تظهر كل الشاشات بالعربية فقط.
...
## إضافة المستخدمين من داخل التطبيق

أصبح زر **إضافة مستخدم** في لوحة المدير يفتح شاشة إنشاء مستخدم جديدة.
الإنشاء يتم عبر Edge Function باسم `create-attendance-user` محمية بـ JWT
وتتحقق من دور المدير، ثم تنشئ حساب Supabase Auth وتضيف الصفوف المرتبطة في
`profiles` و`attendance_users`. لا تضع `service_role` key داخل التطبيق؛
المفتاح يبقى داخل إعدادات Supabase الخاصة بالـ Edge Function.
