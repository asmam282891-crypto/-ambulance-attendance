...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _errorMessage = 'تعذّر تحميل بيانات اللوحة');
    } finally {
  ...
  Future<void> _searchEmployees(String query) async {
    ...
    try {
      final results = await SupabaseService.instance.fetchEmployees(search: query);
      if (!mounted || requestId != _searchRequestId) return;
      setState(() => _employees = results);
    } catch (_) {
      // نتجاهل أخطاء البحث المؤقتة حتى لا نقاطع الكتابة
    }
  }
