...
class _AttendanceReportScreenState extends State<AttendanceReportScreen> {
  ...
  /// يحسب مدة تأخير الموظف عن الوقت الرسمي لبدء الدوام.
  /// يرجع '-' إذا لم يتأخر أو لم يسجل حضورًا.
  String _formatDelay(dynamic checkInValue) {
    if (checkInValue == null) return '-';
    final text = checkInValue.toString().trim();
    if (text.isEmpty) return '-';

    try {
      final checkInTime = DateTime.parse(text).toLocal();
      ...
      final diff = checkInTime.difference(officialStart);
